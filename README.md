...
SDD Tool is a command-line interface designed to work with **Claude Code** for implementing Spec-Driven Development (SDD) methodology. Through **slash commands** and **Skills 2.0**, you collaborate with AI to write specs and implement features.
...
```bash
# 1. Initialize your project (creates slash commands + Skills 2.0 + Git/CI-CD setup)
sdd init

# 2. Start Claude Code
claude

# 3. Begin workflow with slash command or skill
/sdd.start
```
...
Automatically generated in `.claude/commands/` (slash commands, Korean) and `.claude/skills/sdd-*/` (Skills 2.0, English) when you run `sdd init`.

> Each `sdd.foo` slash command has a matching `sdd-foo` skill under `.claude/skills/sdd-foo/SKILL.md`. The slash commands use dot-notation; the skills use kebab-case. Both are generated by default — use `--no-commands` or `--no-skills` to opt out of either.

### Skills 2.0 (v1.6.0+)

Skills 2.0 definitions are authored in English for model-routing accuracy and include advanced frontmatter fields:
...
| `allowed-tools` | All skills — minimum-privilege glob patterns (e.g., `Bash(sdd validate*)`) |
...
│
└── .claude/
    ├── commands/           # Slash commands — Korean, dot-notation (sdd.start.md …)
    │   ├── sdd.start.md
    │   ├── sdd.spec.md
    │   └── ...
    ├── agents/             # Sub-agents
    ...
    │   └── api-scaffold.md
    ├── skills/             # Skills
    │   ├── dev-implement/  # Dev skills (v1.2.0)
    │   │   └── SKILL.md
    │   ├── dev-test/
    │   │   └── SKILL.md
    │   ├── sdd-start/      # SDD workflow skills — English, kebab-case (v1.6.0)
    │   │   └── SKILL.md
    │   ├── sdd-spec/
    │   │   └── SKILL.md
    │   └── ...             # 32 sdd-* skills total
...
> **Skills 2.0 (v1.6.0)**: `sdd init` generates 32 English Skills 2.0 definitions under `.claude/skills/sdd-*/SKILL.md`. These mirror the Korean slash commands and include Skills 2.0 frontmatter (`context`, `allowed-tools`, `disable-model-invocation`). The 6 `dev-*` skills introduced in v1.2.0 are unchanged.
