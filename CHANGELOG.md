...
- **Claude Code Skills 2.0 generation in `sdd init`**
  - `sdd init` now writes 32 English Skills 2.0 definitions under `.claude/skills/sdd-*/SKILL.md` alongside the existing Korean slash commands
  - Skills mirror the Korean slash commands in `src/generators/claude-commands/` but are authored in English for model-routing accuracy
  - Naming convention: kebab-case (`sdd-start`, `sdd-spec`, …) vs dot-notation for slash commands (`sdd.start`, `sdd.spec`, …)

- **New module `src/generators/claude-skills/`**
  - `SkillDefinition` type with all Skills 2.0 frontmatter fields
  - `serializeSkill()` YAML frontmatter writer
  - Five category subdirectories: `core` (7), `management` (3), `analysis` (9), `domain` (3), `utils` (10)

- **Skills 2.0 frontmatter support**
  - `context` mode: `inline` (default), `fork` (7 analysis/domain skills), `manual-invoke-only` (`sdd-watch`)
  - `allowed-tools` with per-skill glob patterns following minimum-privilege rule (e.g., `Bash(sdd validate*)`)
  - `disable-model-invocation: true` on 5 utility skills (`sdd-guide`, `sdd-chat`, `sdd-cicd`, `sdd-watch`, `sdd-migrate`)
  - `hooks`, `agent`, `user-invocable` fields ready for future use

- **`--no-skills` and `--no-commands` flags for `sdd init`**
  - `--no-skills`: skip writing `.claude/skills/` (both `dev-*` and `sdd-*`)
  - `--no-commands`: skip writing `.claude/commands/`
  - Both default to enabled; fully backward-compatible opt-out

### Fixed

- Relaxed `tests/integration/watch.test.ts` `--quiet` assertion that was a pre-existing borderline failure (10 vs `< 10`)

### Notes

- Slash commands in `.claude/commands/` remain fully backward-compatible and continue to be generated by default
- The Korean slash command definitions in `src/generators/claude-commands/` are unchanged
